...
const Signup = () => {
    ...
    if (otpGenerated) {
      ...
      if (password == confirmPassword) {
        if(passwordRegex.test(password)){
          ...
          axios.post(`https://s47-dhruv-capstone-2-craftershub-2.onrender.com/signup`, {
            name,
          username: email,
          password,
          otp,
  ...
  const generateOtp = () => {
    setValidOTP('')
    if (email) {
      axios.post(`https://s47-dhruv-capstone-2-craftershub-2.onrender.com/otpVerification`, { email })
        .then(res => {
